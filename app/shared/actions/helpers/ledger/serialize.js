...
export default function serialize(chainId, transaction, types) {
  ...
  for (let i = 0; i < transaction.actions.length; i += 1) {
    ...
    console.log(`[LEDGER-SERIALIZE] Action ${i} authorization.length:', action.authorization.length);
    encode(
      writer,
    ...
    for (let authIndex = 0; authIndex < action.authorization.length; authIndex += 1) {
      ...
      console.log(`[LEDGER-SERIALIZE] Action ${i}, Auth ${authIndex} permission:', authorization.permission);
      encode(
        writer,
  ...
  console.log('[LEDGER-SERIALIZE] All chunks:', chunks.map(c => c.toString('hex')));
  return chunks;
}
