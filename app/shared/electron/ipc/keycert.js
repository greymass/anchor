...
EncryptedPrivateKey.scrypt = (password, salt, N, r, p, dkLen) =>
  new Promise((resolve, reject) => {
    scrypt(password, salt, dkLen, { N, r, p, maxmem: 1024 * 1024 * 1024 }, (err, derivedKey) => {
      if (err) {
        reject(err);
      } else {
        resolve(derivedKey);
...
function generateNewAccountKeys(store, password, blockchain, accountName, ownerKey = false, activeKey = false) {
  ...
  if (!ownerPublicKey) {
    ...
    store.dispatch(importKeyStorage(password, ownerPrivate.toWif(), ownerPublicKey));
  }
  ...
  if (!activePublicKey) {
    ...
    store.dispatch(importKeyStorage(password, activePrivate.toWif(), activePublicKey));
  }
...
async function getKeycertParams(store, password, chainId, accountName, expectedPublicKey) {
  ...
  if (privateKey.pubkey !== expectedPublicKey) {
    console.log(
      `The expected pubkey didn't match the key in storage, expected: ${expectedPublicKey}, received: ${privateKey.pubkey}. Automatically corrected and exported correct key based on chain results.`
    );
  }
  return {
...
export async function generateKeyCert(store, password, publicKey, chainId, accountName) {
  // Generate the keycert and keys (if required)
  const params = await getKeycertParams(store, password, chainId, accountName, publicKey);
  // Generate key certificate
  const keycert = await generate(params);
  return keycert;
}

export function generateNewAccount(event, store, password, chainId, accountName, ownerKey = false, activeKey = false) {
  ...
  const { active, owner } = generateNewAccountKeys(store, password, blockchain, accountName, ownerKey, activeKey);
...
async function getKeyCertificatePdf(store, cert, chainId) {
  const client = getAPIClient(store, chainId);
...
export async function saveKeyCertificate(event, store, password, publicKey, chainId, accountName) {
  if (!cachedPdf) {
...
export async function printKeyCertificate(event, store, password, publicKey, chainId, accountName) {
  if (!cachedPdf) {
    // Generate the certificate based on the account information provided
    const { cert, encryptionWords } = await generateKeyCert(store, password, publicKey, chainId, accountName);
    // Cache the words in the event the user wants to print after saving
...
export async function updateKeyWithCertificate(event, store, password, certificate, encryptionWords, reset = false) {
  ...
  store.dispatch(importKeyStorage(password, newPrivateKey.toWif(), newPublicKey.toLegacyString(blockchain.keyPrefix)));
  // API Client for use
  const client = getAPIClient(store, chainId);
  // Generate the update auth transaction with or without Fuel
  const updateauth = await generateUpdateAuthTransaction(client, chainId, decrypted.account, newPublicKey, reset);
  ...
  try {
    ...
    event.sender.send('accountUpdatedViaCertificateFailure', {
      accountName: String(decrypted.account.actor),
      chainId,
      publicKey: newPublicKey.toLegacyString(blockchain.keyPrefix),
      error: e,
...
async function generateUpdateAuthTransaction(client, chainId, signer, publicKey, reset = false) {
  ...
  if (reset) {
    ...
    auth.keys = [...auth.keys, newAuth].sort((k1, k2) => (k1.key < k2.key ? -1 : 1)); // sort keys
  }
  // Create transaction
...
async function getFuelSignature(chainId, tx, signer) {
  ...
  if (status === 200) {
    ...
    await ValidateFuel.validateTransaction(signer, requestData, JSON.parse(JSON.stringify(tx)));
