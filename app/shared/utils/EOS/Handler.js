...
const {
  transactionHeader
} = require("eosjs2/node_modules/eosjs/dist/eosjs-serialize");
const {
  stringToPublicKey,
  publicKeyToString
} = require("eosjs2/node_modules/eosjs/dist/eosjs-numeric");
const {
  JsSignatureProvider
...
export default class EOSHandler {
  ...
  push(signedTransaction) {
    ...
    if (!serializedTransaction && signedTransaction.transaction) {
      serializedTransaction = this.api.serializeTransaction(
    ...
    return this.api.pushSignedTransaction({
      signatures,
  ...
  async checkResources(chainId, tx, signer) {
    ...
    if (endpoint) {
      const unsigned = await this.createTransaction(tx, {
        sign: false,
        broadcast: false
      });
      ...
      try {
        const response = await httpClient.post(
          `${endpoint}/v1/resource_provider/request_transaction`,
          {
            request: req.encode(),
            signer
          }
  ...
  async transact(tx, options = false) {
    ...
    if (!combinedOptions.broadcast && !combinedOptions.sign) {
      ...
      const unsigned = await this.createTransaction(
        transaction,
        combinedOptions,
        signatures
      );
    ...
    const processed = await this.customTransact(
      transaction,
      combinedOptions,
      signatures
    );
    ...
    if (!combinedOptions.broadcast) {
      const deserializedTransaction = this.api.deserializeTransaction(
        processed.serializedTransaction
      );
  ...
  createTransaction = async (tx, combinedOptions = [], signatures = []) => {
    ...
    return {
      broadcast: false,
      transaction_id: Serialize.arrayToHex(
        sha256(serializedTransaction)
      ).toLowerCase(),
      transaction: {
        compression: "none",
        signatures,
  ...
  ensureSerializedActions = async actions =>
    (await isObject(actions[0].data))
  ...
  getRequiredAbis = async request => {
    const abis = new Map();
    await Promise.all(
      request.getRequiredAbis().map(async account => {
...
class LedgerSignatureProvider {
  ...
  async sign({ chainId, serializedTransaction }) {
    ...
    if (
      tx.actions[0].name === "identity" &&
      tx.actions[0].authorization.length === 0
    ) {
    ...
    const response = await ledgerApi.signTransaction(
